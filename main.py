...
def rcmd(movie_name):
    ...
    data,cosine_values=create_similarity()
    if movie_name not in data['movie_title'].unique():
        return "Sorry!! Movie is not present in the database"
    else:
        i=data.loc[data['movie_title']==movie_name].index[0]
        movie_list=pd.Series(cosine_values[i,1:11]).sort_values(ascending=False).index
        movie_names=[]
        for i in movie_list:
            movie_names.append(data['movie_title'][i])
    return movie_names
